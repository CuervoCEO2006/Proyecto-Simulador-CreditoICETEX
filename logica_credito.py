...
def calcular_cuota(monto_credito, tasa, plazo):
    ...
    if tasa < 0:
        raise TasaInvalida(f"La tasa de interes ingresada {tasa * 100}% no puede ser negativa")
    ...
    if tasa == 0:
        ...
        return (monto_credito * tasa) / (1 - (1 + tasa) ** (-plazo))


def calcular_total_abonos(monto_credito, tasa, plazo):
    cuota = calcular_cuota(monto_credito, tasa, plazo)
    return cuota * plazo


def calcular_total_intereses(monto_credito, tasa, plazo):
    total_abonos = calcular_total_abonos(monto_credito, tasa, plazo)
